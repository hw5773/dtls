  ...
  unsigned long start, end, stime, etime;
...
#ifdef TIME_LOG
	int sequence;
	uint8_t fname[MAX_FILE_NAME_LEN];
	sequence = 0;
#endif /* TIME_LOG */
	...
	while (running) {
		if ((client = accept(server, (struct sockaddr *) &addr, &len)) > 0) {
			...
			sequence += 1;
			snprintf(fname, MAX_FILE_NAME_LEN, "/home/hwlee/log/%s/origin_%s_%d.csv",
      ...
      ret = getsockopt(client, SOL_SOCKET, SO_SNDBUF, &sndbuf, &optlen);
      if (ret < 0)
        printf("Error getsockopt one\n");
      else
        printf("Send buffer size: %d\n", sndbuf);

      ret = getsockopt(client, SOL_SOCKET, SO_RCVBUF, &rcvbuf, &optlen);
      if (ret < 0)
        printf("Error getsockopt one\n");
      else
      ...
      sndbuf = 81920000;
      ret = setsockopt(client, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf));

      if (ret < 0)
        printf("Error setsockopt\n");

      rcvbuf = 1024;
      ret = setsockopt(client, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf));
